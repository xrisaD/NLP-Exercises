...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        if(transition=="S"):
            ...
            l = len(self.stack)
            first_item = self.stack[l-1]
            second_item = self.stack.pop(l-2)
            self.dependencies.append((first_item, second_item))
        elif(transition=="RA"):
            l = len(self.stack)
            second_item = self.stack[l-2]
            first_item = self.stack.pop(l-1)
...
def minibatch_parse(sentences, model, batch_size):
    ...
    while(unfinished_parses):
        minibatch__partial_parses = unfinished_parses[:+min(batch_size,len(unfinished_parses))]
        transitions = model.predict(minibatch__partial_parses)
        for p,t in zip(minibatch__partial_parses,transitions):
            p.parse_step(t)
