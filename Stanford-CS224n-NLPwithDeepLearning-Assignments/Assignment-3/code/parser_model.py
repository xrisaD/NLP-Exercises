...
class ParserModel(nn.Module):
    ...
    def embedding_lookup(self, w):
        ...
        #x = torch.LongTensor([torch.index_select(self.embeddings, 0, w[i,:]).tolist() for i in range(w.shape[0])])
        batch_size = w.shape[0]
        indices = w.view(batch_size*w.shape[1])
        x = torch.index_select(self.embeddings, 0, indices).view([batch_size, self.n_features * self.embed_size])
        ### END YOUR CODE
        return x
    ...
    def forward(self, w):
        ...
        relu = nn.ReLU()
        hidden = self.dropout(relu(torch.matmul(x, self.embed_to_hidden_weight) + self.embed_to_hidden_bias))
